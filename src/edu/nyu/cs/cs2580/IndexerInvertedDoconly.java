...
public class IndexerInvertedDoconly extends Indexer implements Serializable {
	...
	public void loadIndex(Query query) {
		 ...
		 * query.processQuery(); Vector<String> tokens = query._tokens;
		 * for(String token : tokens) { if(token.trim().length() != 0) {
	...
	public void loadIndex() {
		 ...
		 * System.out.println("Loading indexes from : " +
		 * _options._indexPrefix);
		 * 
		 * File[] indexFiles = indexDir.listFiles(indexFilesFilter);
		 * 
		 * String[] docInfo; Map<Integer, Integer> docInfoMap; Scanner scanner;
		 * String token; String[] doc_count;
		 * 
		 * for (File indexFile : indexFiles) { System.out.println("loading : " +
		 * indexFile.getName()); try { br = new BufferedReader(new
		 * FileReader(indexFile)); line = "";
		 * 
		 * while ((line = br.readLine()) != null) { if (line.trim().length() !=
		 * 0) { scanner = new Scanner(line); scanner.useDelimiter("[" +
		 * _termDoclistDelim + "\n]");
		 * 
		 * token = scanner.next();
		 * 
		 * // create new map entry for current term docInfoMap = new
		 * HashMap<Integer, Integer>(); _invertedIndex.put(token, docInfoMap);
		 * 
		 * // build docInfoMap docInfo = scanner.next().split(_doclistDelim);
		 * for (String doc : docInfo) { doc_count = doc.split(_docCountDelim);
		 * docInfoMap.put(Integer.valueOf(doc_count[0]),
		 * Integer.valueOf(doc_count[1]));
		 * 
		 * _totalTermFrequency += Integer.valueOf(doc_count[1]); } } } } catch
	...
	public int documentTermFrequency(String term, int docId) {
		...
		if (_invertedIndex.containsKey(term)) {
			...
			// check whether the given term is present in the document with
			// given docId
	...
	private void testParse(IndexerInvertedDoconly iido) {
		File corpusFile = new File(iido._options._corpusPrefix
				+ "/The_X_Factor_(Australia)");
		try {
			Document doc = Jsoup.parse(corpusFile, "UTF-8");
